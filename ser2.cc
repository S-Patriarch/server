...
namespace {
   ...
   void* exit_thread_func(void* arg) 
      // функция потока, которая асинхронно основному потоку отслеживает
      // нажатие клавиши [q] для остановки сервера
   { 
      std::int32_t ch {};
      for (;;) {
         // проверяем на необходимость остановки потока
         if (t_stop) break;
            ...
            ch = c.get_ch();
            if (ch=='q') {
               std::cout << "W: stopping the server...\n\n";
               log._slog = dt.get_date()+' '+dt.get_time()+" : "+
                  "stopping the server";
               log._isfwlog = true;
               t_stop = true;
...
std::int32_t main(std::int32_t argc, char** argv)
{
   ...
   uname(&sysinfo);
   std::cout << "SERVER\n";
   std::cout << "- " << sysinfo.sysname << '\n' 
             << "- " << sysinfo.version << '\n';
   std::cout << "\nTo stop the server, press the [q] key\n\n";

   ::tcp.tcp_pthread_create(&::tid_log,NULL,::write_to_log_file,NULL);
   ::tcp.tcp_pthread_create(&::tid_exit,NULL,::exit_thread_func,NULL);
   ...
   try {
      ...
      ::t_stop = true;
      ::tcp.tcp_pthread_join(::tid_log,NULL);
      ::tcp.tcp_pthread_join(::tid_exit,NULL);
   }
